<?php
// ---------- CONFIG ----------
$WORKING_KEY = 'B410D0FB52051326F8B5F33B491A9230'; // CCAvenue key
...
// ---------- DETECT MODE ----------
$mode = (!empty($cc['billing_name']) && !empty($cc['merchant_param1'])) ? 'full_automation' : 'status_update';

// ---------- FUNCTIONS ----------
function updatePaymentStatus($cc){
    $refNo = $cc['order_id'] ?? null;
    if(!$refNo) return;

    $status = strtolower($cc['order_status'] ?? 'failed')==='success' ? 'Success':'Failed';
...
function fullAutomation($cc){
    ...
    // ---------- ACCOUNT ----------
    $account_id = null; $account_name_value = $billing_name;
    ...
    $deal_fields = [
        ...
        "Paymet_Status"=>($stage==='Closed Won'?'Success':'Failed'),
    ...
    log_file('full_automation.json',['deal_id'=>$deal_id,'contact_id'=>$contact_id,'account_id'=>$account_id]);
}

// ---------- PROCESS BASED ON MODE ----------
switch($mode){
    case 'status_update':
        updatePaymentStatus($cc);
        break;
    case 'full_automation':
        fullAutomation($cc);
        break;
    default:
        log_file('unknown_payload.json',$cc);
        break;
}

// ---------- RESPONSE ----------
echo json_encode(["status"=>"ok","mode"=>$mode], JSON_PRETTY_PRINT);
?>
